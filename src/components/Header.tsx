    ...
    <header className="sticky top-0 z-50 bg-[#2D006B] border-b border-[#220052] shadow-md text-white" id="main-header">
      ...
      <div className="hidden lg:block max-w-7xl mx-auto px-4 md:px-6 py-3">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {/* Search Bar */}
          <div className="flex-1 max-w-xs xl:max-w-sm relative">
          ...
          <div className="flex items-center gap-2.5 xl:gap-3 flex-shrink-0">
            {/* Branch Selector */}
            <div className="relative flex items-center gap-1 bg-white/10 border border-white/20 rounded-full px-2.5 py-1 hover:bg-white/15 transition-colors">
              <Building className="w-3 h-3 text-white flex-shrink-0" />
              <select
            ...
            {/* Navigation Links */}
            <nav className="flex items-center gap-3 xl:gap-4 text-[11px] font-bold uppercase tracking-wider text-white/80">
              <button
                onClick={() => handleTabClick('home')}
            ...
            {/* User Auth controls */}
            {user ? (
              <div className="flex items-center gap-2 bg-white/10 border border-white/20 rounded-full py-1 pl-1.5 pr-2.5">
                {user.photoURL ? (
                  <img
                    src={user.photoURL}
                  ...
                  <div className="w-6 h-6 rounded-full bg-red-600 text-white font-black text-[10px] flex items-center justify-center">
                    {user.email?.[0].toUpperCase() || "U"}
                  </div>
                )}
                <div className="text-left hidden 2xl:block">
                  <p className="text-[10px] font-black text-white truncate max-w-[90px] leading-none">
                    {user.displayName || user.email?.split('@')[0]}
                  </p>
                  <span className="text-[7.5px] font-black text-[#80CBC4] uppercase tracking-widest block mt-0.5">Patient</span>
                </div>
                <button
                  onClick={logout}
              ...
              <button
                onClick={() => handleTabClick('bookings')}
                className="flex items-center gap-1 px-3 py-1.5 border border-white/20 hover:bg-white/10 text-white rounded-full text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer shadow-xs"
              >
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3.5 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer ${currentTab === 'admin'
            ...
            <a
              ...
              className="flex items-center gap-1 px-3.5 py-1.5 bg-[#AD1457] hover:bg-[#C2185B] text-white rounded-full text-[11px] font-bold uppercase tracking-wider transition-all shadow-md cursor-pointer"
